...
TEST(PubSubTest, SubscriberAutoCleanup) {
  ...
  EXPECT_NO_THROW({ ps.Publish<int>("cleanup_topic", 2); });
}
...
TEST(PubSubTest, OverflowPolicyDropOldest) {
  PubSub ps;
  auto sub =
      ps.Subscribe<int>("drop_oldest_topic", 2, OverflowPolicy::DropOldest);
...
TEST(PubSubTest, OverflowPolicyDropNewest) {
  PubSub ps;
  auto sub =
      ps.Subscribe<int>("drop_newest_topic", 2, OverflowPolicy::DropNewest);
...
TEST(PubSubTest, TypeMismatchException) {
  ...
  EXPECT_THROW({ ps.Publish<double>("type_topic", 3.14); },
...
TEST(SelectorTest, MultipleTypes) {
  ...
  selector.Add<std::string>(
      subStr, [&str_val](const std::string& val) { str_val = val; });
...
TEST(WorkerTest, BasicWorker) {
  ...
  worker.SetTickCallback(std::chrono::milliseconds(10),
                         [&ticks]() { ticks++; });
...
TEST(PubSubTest, SharedPtrMessageCleanup) {
  PubSub ps;
  auto sub = ps.Subscribe<std::shared_ptr<int>>("shared_ptr_topic", 2,
...

  {
    ...
    ps.Publish<std::shared_ptr<int>>("shared_ptr_topic",
                                     ptr3);  // ptr1 should be dropped
  }
...
TEST(SelectorTest, SubscriberDestructionNoBusyLoop) {
  ...
  // Keep one subscriber alive so the selector has at least one active channel
  // to wait on
  ...
  // Call WaitFor. If the busy loop bug exists, this will return immediately due
  // to POLLNVAL/invalid handle. When fixed, it should clean up the expired
  // subscriber and block for the full timeout on the alive subscriber.
  auto start = std::chrono::steady_clock::now();
  bool processed = selector.WaitFor(std::chrono::milliseconds(50));
  auto end = std::chrono::steady_clock::now();
  ...
  auto duration_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
          .count();
  EXPECT_FALSE(processed);
  // It should have waited for the full timeout since no events are pending on
  // the alive subscriber
...
TEST(PubSubTest, ZeroCapacityException) {
  PubSub ps;
  EXPECT_THROW({ ps.Subscribe<int>("zero_capacity", 0); },
               std::invalid_argument);
}
...
TEST(PubSubTest, TopicLifecycle) {
  ...
  // Publishing to "lifecycle_topic" now will create a NEW topic, so "sub" won't
  // get it
...
// Test that publishing a move-only type with multiple active subscribers throws
// an exception
TEST(PubSubTest, MoveOnlyTypeMultipleSubscribersThrows) {
  ...
  EXPECT_THROW({ ps.Publish("move_only_multi", std::move(ptr)); },
               std::runtime_error);
}
...
struct CustomMoveOnly {
  ...
  CustomMoveOnly clone() const { return CustomMoveOnly(val); }
};

// Test that move-only types with a clone() method can be published to multiple
...
// Another move-only type without clone() that uses template specialization of
// Cloner
...
// Test that move-only types with specialized Cloner can be published to
// multiple subscribers successfully
...
// Test that Selector supports move-only type callbacks and successfully
// transfers ownership
TEST(SelectorTest, MoveOnlyTypeCallback) {
  ...
  selector.Add<std::unique_ptr<int>>(sub,
                                     [&received_ptr](std::unique_ptr<int> ptr) {
...
// Test that Selector correctly handles extremely large timeout values without
// overflow/indefinite wait
TEST(SelectorTest, LargeTimeoutClamping) {
  ...
  selector.Add<int>(sub,
                    [&received_val](const int& val) { received_val = val; });
  ...
  // Clamping to a max safe value ensures it successfully waits and wakes up
  // when the thread publishes.
