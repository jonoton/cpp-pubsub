...
#ifdef _WIN32
namespace detail {
inline VOID CALLBACK SelectorWaitCallback(PVOID lpParam,
...
class Subscriber {
 ...
 public:
   ...
   */
  Subscriber(size_t max_capacity = 1000,
    ...
    if (!event_) throw std::runtime_error("Failed to create Windows Event");
#else
    if (pipe(pipe_fds_) != 0)
  ...
  void push(const T &message) {
    static_assert(std::is_copy_constructible_v<T>,
                  "push(const T&) requires copy-constructible type T. Use "
                  "push(T&&) for move-only types.");
   ...
   * @return An optional containing the message if the queue was not empty,
  ...
  std::optional<T> try_receive() {
...

    {
      ...
      if (queue_.empty()) {
        ...
        ResetEvent(event_);
#else
        // Clear the pipe so poll stops triggering (inside lock to prevent
        // races)
        char c;
    ...
    if (popped) cv_space_.notify_one();
...
struct Cloner {
  ...
  static auto perform(const U &val) -> typename std::enable_if_t<
      std::is_copy_constructible_v<U> || has_clone<U>::value, U> {
...
template <typename T>
struct is_cloneable<
    T, std::void_t<decltype(Cloner<T>::perform(std::declval<const T &>()))>>
    : std::true_type {};
...
class Topic : public TopicBase {
 ...
 public:
  ...
  void Publish(T &&message) {
    ...
    if (active_subs.empty()) return;

    if (active_subs.size() == 1) {
      // Zero-overhead direct move for a single subscriber (works for copyable
      // and move-only types)
    ...
    if constexpr (is_cloneable_v<T>) {
      ...
      throw std::runtime_error(
          "Cannot publish move-only type to multiple subscribers without a "
          "Cloner/clone mechanism");
    }
  }
};

/**
 * @brief A publisher object that allows bypassing the broker map lookup for
 * high performance.
 * @tparam T The message type.
 */
template <typename T>
class Publisher {
 ...
 public:
  explicit Publisher(std::shared_ptr<Topic<T>> topic)
   ...
   * @param message The message to publish.
   */
  void Publish(T &&message) { topic_->Publish(std::move(message)); }
};
...
class PubSub {
 ...
 public:
  ...
  std::shared_ptr<Subscriber<T>> Subscribe(
      const std::string &name, size_t capacity = 1000,
      OverflowPolicy policy = OverflowPolicy::Block) {
  ...
  void Unsubscribe(const std::string &name,
                   const std::shared_ptr<Subscriber<T>> &sub) {
    std::lock_guard<std::mutex> lock(map_mutex_);
    auto it = topics_.find(name);
    if (it != topics_.end()) {
      auto topic = std::dynamic_pointer_cast<Topic<T>>(it->second);
...
class Selector {
 ...
 public:
  ...
  void Add(std::shared_ptr<Subscriber<T>> sub, F &&callback) {
    ...
    target->is_expired = [weak_sub]() -> bool { return weak_sub.expired(); };
    target->process_all = [weak_sub,
                           callback = std::forward<F>(callback)]() -> bool {
      auto locked_sub = weak_sub.lock();
      if (!locked_sub) return false;  // Subscriber is dead
  ...
  bool WaitFor(const std::chrono::duration<Rep, Period> &timeout) {
    ...
    if (timeout >=
      ...
      if (timeout_ms == 0 &&
        ...
        timeout_ms = 1;  // Round up sub-millisecond positive values to prevent
                         // zero-timeout busy loops
      }
    }
...

    {
      ...
      // Pre-pass: check for expired subscribers to prevent dead locks / busy
      // loops on invalid fds/handles
      ...
      if (dirty_) {
        ...
        for (const auto &t : targets_) cached_handles_.push_back(t->handle);
#else
        cached_fds_.clear();
    ...
    if (handles.size() <= MAXIMUM_WAIT_OBJECTS) {
      ...
      // When waiting on more than 64 handles, we fallback to the Windows Thread
      // Pool API to avoid busy-waiting. This carries higher latency and
      // overhead. For optimal performance on Windows, distribute subscriptions
      // across multiple Worker instances to keep each under the 64-handle
      // limit.
      HANDLE wakeup_event = CreateEvent(NULL, TRUE, FALSE, NULL);
      if (!wakeup_event)
        throw std::runtime_error(
            "Failed to create Windows Event for fallback threadpool "
            "registration");
      ...
      for (size_t i = 0; i < handles.size(); ++i) {
        if (!RegisterWaitForSingleObject(
                &wait_handles[i], handles[i], &detail::SelectorWaitCallback,
                wakeup_event, INFINITE, WT_EXECUTEONLYONCE)) {
...
class Worker {
 ...
 public:
   ...
   */
  void SetTickCallback(std::chrono::milliseconds timeout,
                       std::function<void()> callback) {
    std::lock_guard<std::mutex> lock(worker_mutex_);
    timeout_ = timeout;
