...
namespace cpppubsub {
...
#else
            if (pipe(pipe_fds_) != 0) throw std::runtime_error("Failed to create POSIX pipe");
        ...
        void push(const T& message) {
            {
                ...
                if (queue_.size() >= max_capacity_) {
                    if (overflow_policy_ == OverflowPolicy::Block) {
                        cv_space_.wait(lock, [this]() { return queue_.size() < max_capacity_ || closed_; });
                        if (closed_) return;
        ...
        std::shared_ptr<Topic<T>> GetOrCreateTopic(const std::string& name) {
            {
                ...
                if (it != topics_.end()) {
                    ...
                    if (!topic) throw std::invalid_argument("Type mismatch on topic: " + name);
                    return topic;
                }
            }
    ...
    public:
        ...
        bool WaitFor(const std::chrono::duration<Rep, Period>& timeout) {
            ...
            for (const auto& t : targets_) handles.push_back(t.handle);
            ...
            for (const auto& t : targets_) fds.push_back({t.fd, POLLIN, 0});

            int result = poll(fds.data(), fds.size(), static_cast<int>(timeout_ms));
            if (result > 0) {
                for (size_t i = 0; i < fds.size(); ++i) {
                    if (fds[i].revents & POLLIN) targets_[i].process_all();
                }
                return true;
            }
            return false;
