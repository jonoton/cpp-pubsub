...
int main() {
  ...
  // We restrict the queue size to 2, and drop the oldest messages if it
  // overflows.
  auto sensor_sub = pubsub.Subscribe<SensorData>(
      "sensors", 2, cpppubsub::OverflowPolicy::DropOldest);
  ...
  std::thread event_loop([&]() {
    ...
    selector.Add<SensorData>(sensor_sub, [](const SensorData& data) {
      print_safe("[Event Loop] Processed Sensor " +
                 std::to_string(data.sensor_id) +
  ...
  // if the event loop doesn't read them fast enough, older ones will be
  // dropped.
  for (int i = 1; i <= 5; ++i) {
